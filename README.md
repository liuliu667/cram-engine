...
① **课程名称？**
② **使用什么教材？** 没有就跳过。填了的话引擎会用该教材的术语体系，更贴近课堂表述。
③ **有复习资料吗？** 支持粘贴 .txt / .md 内容或给文件路径。没有就跳过。
④ **考试有哪些题型？** 用你自己的话描述，老师说什么就写什么。
⑤ **列出要考的知识点**，每行一个。
⑥ **以上哪些是老师反复强调或说过会考的？** 回复编号，如 1, 3-5。没有就说没有。
⑦ **其他知识点里，有没有你自己觉得需要加强的？** 有就标编号，没有就跳过。

这七个问题里最关键的是最后两个——它们把知识点分为「必考点」和「扩展点」两层，这个双层优先级贯穿全部四个阶段：必考点在讲授阶段优先覆盖、检题阶段所有题型全覆盖、补漏阶段答错多测；扩展点则抽样覆盖以节省时间。引擎在有限时间内把火力集中在最可能考的内容上，而不是对所有知识点平均用力。
